...
import { closeSidebar } from "../../store/slices/authSlice";
...
const validateRoutes = (routes) => {
  ...
  return routes.filter(route => {
    // Check if the route has a valid component
...
const Sidebar = ({ open, onClose }) => {
  ...
  // IMPORTANT: Only use Redux state for controlling the sidebar
  // We no longer mix props and Redux state to avoid confusion

  useEffect(() => {
    console.log("%c[Sidebar DEBUG] Redux sidebarOpen:", "background:red;color:white;font-weight:bold;padding:3px 5px", sidebarOpen);

    // Force a proper DOM update when sidebar state changes
    if (sidebarRef.current) {
      // This triggers a repaint
      const width = sidebarRef.current.offsetWidth;

      // Apply a forced visible class when open
      if (sidebarOpen) {
  ...
  const handleClose = (e) => {
    ...
    console.log("%c[Sidebar] Closing sidebar...", "background:red;color:white;font-weight:bold;padding:3px 5px");
    dispatch(closeSidebar());

    // Also call the prop-based handler if available (for compatibility)
    ...
    <>
      ...
      <div
        ...
        style={{
          ...
          visibility: sidebarOpen ? 'visible' : 'visible',
      ...
      {/* Debug sidebar state indicator (always visible) */}
      <div className="fixed bottom-4 left-4 z-50 bg-black text-white px-3 py-1 text-xs rounded-full">
